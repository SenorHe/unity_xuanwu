...
namespace UnityTcp.Editor.Native
{
    ...
    internal static partial class NativeWindowBridgeHost
    {
        ...
        /// <summary>
        /// Save {slotId → Window.instanceId} mapping so surviving windows
        /// can be rediscovered after domain reload without closing them.
        /// DockArea serializes m_Panes/m_Selected automatically, so the
        /// active tab is preserved without SaveCompositeActiveTab.
        /// </summary>
        private static void SaveCompositeSlotMapping()
        {
            ...
            // Build a simple JSON array: [{"s":"slotId","i":instanceId}, ...]
            var sb = new System.Text.StringBuilder("[");
            bool first = true;
            foreach (var kvp in s_CompositeSlots)
            {
                if (kvp.Value?.Window == null) continue;
                if (!first) sb.Append(',');
                first = false;
                sb.Append("{\"s\":\"");
                sb.Append(kvp.Key.Replace("\"", "\\\""));
                sb.Append("\",\"i\":");
                sb.Append(kvp.Value.Window.GetInstanceID());
                sb.Append(",\"t\":\"");
                sb.Append(kvp.Value.WindowTypeName?.Replace("\"", "\\\"") ?? "");
                sb.Append("\"}");
            }
            sb.Append(']');
            SessionState.SetString(kCompositeSlotMappingKey, sb.ToString());
            LogVerbose($"[NWB-Composite] Saved slot mapping ({s_CompositeSlots.Count} slots) before reload");
        }

        /// <summary>
        /// After domain reload, rediscover surviving EditorWindows from the
        /// saved mapping and pre-populate s_CompositeSlots. When the browser
        /// re-sends the composite layout, ApplyCompositeLayout will find
        /// existing slots and reuse them instead of creating new windows.
        /// </summary>
        private static void PrePopulateCompositeSlots()
        {
            ...
            // Minimal JSON parse: extract {s, i, t} entries
            int restored = 0;
            int idx = 0;
            while (idx < json.Length)
            {
                ...
                if (objEnd < 0) break;
                string entry = json.Substring(objStart, objEnd - objStart + 1);
                idx = objEnd + 1;

                string slotId = ExtractJsonStringValue(entry, "s");
                string typeName = ExtractJsonStringValue(entry, "t");
                int instanceId = ExtractJsonIntValue(entry, "i");
        ...
        private static void OnBeforeAssemblyReload()
        {
            ...
            // Composite mode: save slot mapping and release RTs, but keep
            // EditorWindows alive. DockArea serializes m_Panes/m_Selected,
        ...
        private static bool StartCompositeOffscreenCapture(string requestJson, int fps, int width, int height)
        {
            ...
            try
            {
                ...
                s_OffscreenFps = fps;
                // Only reset frame count on FIRST composite start, not when
                // adding/updating slots. Resetting triggers warm-up which
                // calls FocusAndRepaintCompositeGameView → steals active tab
                // back to GameView, preventing newly added views from showing.
                ...
                // Clear target so initial frames use PrintWindow (not GrabPixels).
                // Input injection will re-set it on the first user interaction.
                s_OffscreenTarget = null;
                s_OffscreenTargetType = null;
                s_NextCaptureTime = 0;
                s_HasFrontendHeartbeat = false;
                ...
                ShowStreamingMaskWindow();
#endif
                // Snapshot current EditorWindow set so new popups can be detected
        ...
        private static CompositeCaptureSlot CreateCompositeSlot(string slotId, string windowTypeName, int logicalW, int logicalH, CompositeRect rect)
        {
            ...
            // Add subsequent slots as tabs in the existing DockArea.
            // The user can drag a tab out to create a SplitView within the
            // same ContainerWindow; CaptureAndPushCompositeFrame handles this.
        ...
        private static void ReacquireCompositeSlotHandles(CompositeCaptureSlot slot)
        {
            ...
            if (slot.ContainerHwnd == IntPtr.Zero)
            {
                slot.ContainerHwnd = Cn.Tuanjie.Codely.Editor.NativeWindowHelper.GetHWND(slot.Window);
                if (slot.ContainerHwnd != IntPtr.Zero)
                {
                    IntPtr child = GetWindow(slot.ContainerHwnd, GW_CHILD);
                    if (child != IntPtr.Zero)
            ...
            if (slot.GUIViewHwnd == IntPtr.Zero || !IsWindow(slot.GUIViewHwnd))
            {
            ...
            LogVerbose($"[NWB-Composite] Reacquired slot '{slot.SlotId}' ContainerHwnd=0x{slot.ContainerHwnd.ToInt64():X}, GUIViewHwnd=0x{slot.GUIViewHwnd.ToInt64():X}");
#endif
#if UNITY_EDITOR_OSX
            ...
            foreach (var pair in viewSlots)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (usePrintWindow)
                        {
                            captured = TryCaptureViaPrintWindow(printWindowHwnd, s_CaptureRT);
        ...
        // Tracks whether Play-mode focus initialization has been performed
        // for the current Play session.  Reset when isPlaying becomes false.
        // After domain reload the default (false) triggers re-initialization.
        private static bool s_CompositePlayModeInitDone;

        // After a mousedown in composite Play mode, force synchronous repaint
        // for a few frames to eliminate the 1-frame backbuffer lag that causes
        // a brief compression flash.
                ...
                if (evt != null)
                {
                    ...
                    else if (!sentToPopup)
                    {
                        ...
                        else
                        {
                            ...
                            // Only block tab-drag SendEvent on Unity 2019
                            // (Gamma / PrintWindow mode) where DragTab →
                            // GetOrderedWindowList crashes from dangling PPtr.
                            // Tuanjie (Linear) supports tab drag with
                            // DrawPaneDragTabOverlay and does not crash.
                            if (!skipSendEvent && type == "mousedrag"
                                && s_MouseDownInTabBar && s_UseRenderTextureFallback)
                                skipSendEvent = true;
                            bool used = false;
                            if (!skipSendEvent)
                            {
        ...
        private static bool TryCaptureViaPrintWindow(IntPtr containerHwnd, RenderTexture captureRT)
        {
            ...
            if (s_PrintWindowLogCount <= 3 || s_PrintWindowLogCount % 600 == 0)
            {
                LogVerbose($"[NWB-PrintWin] #{s_PrintWindowLogCount} winRect={winW}x{winH} client={clientW}x{clientH} " +
                    $"border=({borderL},{borderT}) dst={dstW}x{dstH} copy={copyW}x{copyH} layered={wasLayered} dpi={DPIScale}");
            }
