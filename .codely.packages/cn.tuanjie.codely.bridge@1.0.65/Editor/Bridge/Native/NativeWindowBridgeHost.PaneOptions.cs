...
namespace UnityTcp.Editor.Native
{
    internal static partial class NativeWindowBridgeHost
    {
        ...
        /// <summary>
        /// DockArea title-bar band: tabs plus the PaneOptions (⋮) button drawn at
        /// tabArea.y + genericMenuTopOffset (~20px below the 19px tab strip).
        /// </summary>
        private static bool IsInDockTitleBarRegion(float mouseY)
        {
            float tabStripH = Mathf.Max(s_TabBarOffsetY, kDockTabStripHeight);
            float headerBottom = tabStripH + kGenericMenuTopOffset + kPaneOptionsButtonHeight + 4f;
            return mouseY >= -2f && mouseY <= headerBottom;
        }

        private static bool IsPaneOptionsClick(float mouseX, float mouseY, float viewWidth)
        {
            if (!IsInDockTitleBarRegion(mouseY)) return false;
        ...
        private static bool TryInterceptPaneOptionsClick(float mouseX, float mouseY, float viewWidth, float ppp)
        {
            ...
            try
            {
                ...
                float tabStripH = Mathf.Max(s_TabBarOffsetY, kDockTabStripHeight);
                float popX = mouseX;
                float popY = Mathf.Max(tabStripH + kGenericMenuTopOffset + 2f, mouseY + 2f);
